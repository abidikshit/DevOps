...
def del_operation():
    with open(f_input, 'r') as inactive_users:
        for userid in inactive_users:
            ...
            def service_account():
                check_service_account = "p4 groups -u "+user+" " \
                                                             "|egrep 'service_users|serviceaccounts|" \
                                                             "serviceperforce|serviceperforceread'"
                service_account_status = os.system(check_service_account)
                if service_account_status == 0:
                    print("'"+user+"' belongs to service account, excluding...")
                else:
                    print("'"+user+"' does not belongs to service account, checking for normal/super user...")
                    superuser()

            def superuser():
                check_superuser = "p4 protect -o | tail -12 | grep -w "+user
            ...
            def user_del():
                ...
                if user_status:
                    # print("\n'" + user + "' deletion failed. Checking for shelved files...")
            ...
            def workspace_del():
                check_clients = "p4 changes -s pending -u "+user+" |awk {'print $6'}|cut -d '@' -f2 >"+f_mid
            ...
            def check_open_files():
                ...
                if status_opened:
                    # print("Deleting opened files...")
                    open_del = subprocess.Popen("p4 -z maxScanRows=2000000 opened "
                                                "-u"+user+"|cut -d '@' -f2 |awk {'print $1'}| awk '!x[$0]++' >"+f_last,
